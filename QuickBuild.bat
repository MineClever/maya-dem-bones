...
:: into that Maya's Python environment.
::
:: Python 3 path : scikit-build via pip install .
:: Python 2.7 path: cmake direct + setup_prebuilt.py install
::
...
if errorlevel 1 (
    echo [QuickBuild] cmake configure failed.
    pause & exit /b 1
)
...
if errorlevel 1 (
    echo [QuickBuild] cmake build failed.
    pause & exit /b 1
)
