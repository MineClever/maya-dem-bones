...
def validate_selection():
    ...
    sel.getDagPath(sel.length() - 1, vtx_anim_mesh_dag)
    ...
    if sk_mesh_fn.numVertices() != vtx_anim_mesh_fn.numVertices():
        OpenMaya.MGlobal.displayWarning(
            "Vertex count mismatch: <%s> vs <%s>: <%d> vs <%d>" %
            (sk_mesh_dag.partialPathName(), vtx_anim_mesh_dag.partialPathName(),
             sk_mesh_fn.numVertices(), vtx_anim_mesh_fn.numVertices())
        )
        return None, None
...
def run_dem_bones(source_mesh: str, target_mesh: str, start_frame: int, end_frame: int,
    ...
    db.bind_update = 2 # Update bind pose before compute
